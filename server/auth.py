...
def validate_token(token):
    ...
    try:
        ...
        return (
            ...
            if User.query.get(decoded_token.get("user_id"))
            else None
        )
...
def validate_password(password):
    return (
        sum(c.isalpha() for c in password) >= 5
...
def register_user():
    ...
    if not validate_password(password):
        return error_response(
            "Password must contain at least 3 numbers and 5 letters", 400
        )
...
def edit_user(user_id):
    ...
    if "user_name" in data:
        ...
        if User.query.filter(
            User.user_name == new_username, User.user_id != user_id
        ).first():
    ...
    if "is_admin" in data:
        user.is_admin = data["is_admin"] in [
            "true",
            "True",
            True,
        ]  # ✅ Convert properly
    ...
    return (
        jsonify(
            {"message": "User profile updated successfully", "user": user.to_dict()}
        ),
        200,
    )
...
def add_book():
    ...
    existing_book = Book.query.filter(
        (Book.title == title) | (Book.isbn == isbn)
    ).first()
    if existing_book:
...
def purchase_book(slug):
    ...
    receipt = Receipt(
        user_id=user_id, book_id=book.book_id, written_time=datetime.utcnow()
    )
...
def get_book_reviews(slug):
    ...
    return (
        jsonify({"message": "Reviews retrieved successfully", "reviews": reviews}),
        200,
    )
...
def get_admin_requests():
    ...
    return (
        jsonify(
            {"message": "Admin requests retrieved successfully", "requests": requests}
        ),
        200,
    )
...
def submit_admin_request():
    ...
    if not user or not user.is_active or user.is_admin:
        return error_response(
            "Unauthorized: Only active non-admin users can request admin status", 403
        )
...
def review_admin_request(request_id):
    ...
    if not user or not user.is_admin:
        return error_response(
            "Unauthorized: Only admins can review admin requests", 403
        )
    ...
    if decision not in [True, False]:
        return error_response(
            "Invalid decision value. Must be 'True' (accept) or 'False' (reject)", 400
        )
